...
public class DocumentClassifierView extends FrameView implements PreferenceChangeListener {
 ...
 //       fileMenu.add(trainingMenuItem);
    ...
    /**
     * Action associated with the menÃ¹ entry File->Open: It shows a JFileChooser, used to choose
    ...
    /**
     * This method handles the ActionEvent generated when the user clicks, or presses 'Enter', on the menÃ¹ entry 'Modify->Preferences':
            ...
            try {
                ...
                if(visualizeListDocuments) {
                    ...
                    for (int i = 0; i < infoDocuments.size(); i++) {
                         ...
                         */
                        infoRow.add(String.format("%6.5f",infoDocument.get("Distance")) + (metric.equals("TF-IDF") ? " ("+String.format("%4.2f",infoDocument.get("Angle"))+"Â°)" : ""));
            ...
            for (int i = KNNMinimum; i <= KNNMaximum; i++) {
                ...
                for ( Set<Document> currentControlSet : partitionedTrainingSet ) {   //2.1
                    ...
                    for(Document currentQuery: currentControlSet) {             //2.1.2
                        ...
                        currentMessage=documentClassifierViewResources.getString("messages.Document")+" nÂ° "+indexCurrentDocument+"/"+foldDimension+" (fold nÂ° "+indexCurrentFold+"): "+documentClassifierViewResources.getString("messages.estimatedCategory")+": '"+calculatedCategory+"', "+documentClassifierViewResources.getString("messages.effectiveCategories")+": '"+effectiveCategories+"' - "+documentClassifierViewResources.getString("messages.currentNumberErrors")+": "+errorCounter;
                        textAreaOutput.append(currentMessage+"\n");
                        if(isLogging) {
                            infoLogger.info(currentMessage);
                        }
